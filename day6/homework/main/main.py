...
try:
    with open(current_dir+"\\database\\schools.json","r") as fh_schools, \
         open(current_dir+"\\database\\courses.json","r") as fh_courses:
         list_school = json.load(fh_schools)
         list_course = json.load(fh_courses)
except:
    pass
...
def createSchool(name,location):
    ...
    with open(current_dir+"\\database\\schools.json","w") as fh_schools:
        json.dump(list_school,fh_schools)
...
def createCourse(name,price,period):
    ...
    with open(current_dir+"\\database\\courses.json","w") as fh_courses:
        json.dump(list_course,fh_courses)
...
def updateSchools(obj):
    with open(current_dir + "\\database\\schools.json", "w") as fh_schools:
        json.dump(obj,fh_schools)

def getCourses():
    with open(current_dir + "\\database\\courses.json", "r") as fh_courses:
        return json.load(fh_courses)
